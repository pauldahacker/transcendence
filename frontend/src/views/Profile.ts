import { renderUser } from '@/renderProfile/RendersUser'
import { renderStats } from '@/renderProfile/RenderStats'
import { renderHistory } from '@/renderProfile/renderHistory'
...
export async function renderProfile(
  ...
  const loggedUsername = getUsernameFromToken(token);
  if (!loggedUsername || loggedUserId === null) {
    localStorage.removeItem("auth_token");
    window.location.hash = "#/login";
    return;
  }

  // Determine if viewing own or another user's profile
  const isOwnProfile = !userIdParam || userIdParam === loggedUserId;
  const readonly = options.readonly || !isOwnProfile;

  const viewedUserId = userIdParam ?? loggedUserId;
  console.log(`Rendering profile for user ID: ${viewedUserId} (own=${isOwnProfile})`);
  ...
  try {
    stats = await getUserStats(viewedUserId);
    data = await getUserData(viewedUserId);
    ranks = await getUserRank(viewedUserId);
    tournamentWins = await getTournamentWins(viewedUserId);
  } catch (err) {
  ...
  const avatarUrl =
    data?.avatar_url && data.avatar_url.trim() !== "" && data.avatar_url !== "null"
      ? data.avatar_url
      : new URL("../imgs/avatar.png", import.meta.url).href;

  const username = data?.username ?? loggedUsername;
  ...
  const rank = ranks || 0;
  const tournaments = tournamentWins ?? 0;
  ...
  renderLastMatches(viewedUserId);

  // Editing / interactive logic (only for own profile)
  if (!readonly) {
    setupAvatarPopup(loggedUserId);
    setupDisplayNameEditor(loggedUserId, data.display_name || username);
    setupBioButton(loggedUserId, bio);
