export function renderStats(wins: number, losses: number, friends: number, tournaments: number, rank:number, medalUrl: string) : string{
  ...
  return `
    ...
    <div class="flex flex-col gap-[3vh] w-[30vw] h-full">
      ...
      <!-- Game Stats -->
      <div class="flex flex-col items-center bg-cyan-900/30 border-4 border-cyan-800 rounded-3xl p-[3vh] shadow-xl flex-1">
      ...
      <!-- Social & Rank -->
      <div class="flex gap-[3vh] flex-1">
        <!-- Friends -->
        <div class="bg-cyan-900/30 border-4 border-cyan-800 text-center rounded-3xl p-[3vh] shadow-xl flex-1">
          <h3 class="font-bit text-[3vh] text-gray-100 mb-[2vh]">Friends</h3>
          <div class="flex flex-col justify-start items-center">
            <span class="font-bit text-[5vh] text-cyan-200">${friends}</span>
          </div>
        </div>

        <div class="bg-cyan-900/30 border-4 border-cyan-800 text-center rounded-3xl p-[3vh] shadow-xl flex-1">
          <h3 class="font-bit text-[3vh] text-gray-100 mb-[2vh]">Rank</h3>
          <div class="flex flex-col justify-start items-center">
            <span class="font-bit text-[5vh] text-amber-300">#${rank}</span>
          </div>
        </div>
      </div>
    </div>
